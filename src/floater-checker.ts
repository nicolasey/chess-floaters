...
export function canFloat(
  ...
  protection = 2) {
    for (let index = playerHistory.length; index > playerHistory.length - protection; index--) {
      if (playerHistory[index].floater && playerHistory[index].floater === direction) return false;
    }
    return true;
}
